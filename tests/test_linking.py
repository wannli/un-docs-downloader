...
class TestLinkDocuments:
    ...
    def test_symbol_link_takes_precedence(self):
        """Symbol-based linking takes precedence over other candidates."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
    ...
    def test_proposal_already_linked_not_relinked(self):
        """Proposals already linked to a resolution are not re-linked."""
        documents = [
            {
                "symbol": "A/RES/80/1",
                "title": "Climate Action",
...
class TestLinkageIntegration:
    ...
    def test_full_link_and_annotate_workflow(self):
        ...
        documents = [
            ...
            # Resolution without symbol reference (should remain unlinked)
            {
                "symbol": "A/RES/80/2",
                "doc_type": "resolution",
            ...
            # Base proposal (same agenda item)
            {
                "symbol": "A/80/L.5",
                "doc_type": "proposal",
        ...
        assert len(res2["linked_proposals"]) == 0
        ...
        assert l5["is_adopted_draft"] is False
        assert l5["adopted_by"] is None
